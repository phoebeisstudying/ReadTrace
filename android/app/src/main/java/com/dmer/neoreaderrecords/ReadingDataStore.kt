...
object ReadingDataStore {
    ...
    fun queryDailyTotals(
        ...
        return runCatching {
            ...
            AutoRefreshLog.e(context, "ReadingDataStore daily totals query failed source=$source range=$startDate~$endDate", it)
            emptyList()
        }
    }

    fun queryPeriodBooks(
        context: Context,
        source: String,
        startDate: String,
        endDate: String
    ): List<PeriodBookRecord> {
        return runCatching {
            val out = mutableListOf<PeriodBookRecord>()
            Helper(context.applicationContext).readableDatabase.use { db ->
                db.query(
                    TABLE_PERIOD_BOOKS,
                    arrayOf(
                        "period_start", "period_end", "source", "book_key", "title", "author",
                        "cover_cache_path", "duration_ms", "confidence", "last_seen_at"
                    ),
                    "source = ? AND period_end >= ? AND period_start <= ?",
                    arrayOf(source, startDate, endDate),
                    null,
                    null,
                    "period_start ASC, duration_ms DESC"
                ).use { c ->
                    while (c.moveToNext()) {
                        out.add(
                            PeriodBookRecord(
                                periodStart = c.getString(0),
                                periodEnd = c.getString(1),
                                source = c.getString(2),
                                bookKey = c.getString(3),
                                title = c.getString(4),
                                author = if (c.isNull(5)) null else c.getString(5),
                                coverCachePath = if (c.isNull(6)) null else c.getString(6),
            ...
            AutoRefreshLog.e(context, "ReadingDataStore period books query failed source=$source range=$startDate~$endDate", it)
            emptyList()
        }
    }
